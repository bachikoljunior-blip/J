...
class BlockActionKernelFactorization:
    ...
    block_count: int
    generator_count: int
    estimated_generator_point_checks: int
    work_cap: int
...
def certify_block_action_kernel_factorization(
    ...
    *,
    ...
    try:
        ...
        checks = 2 * generator_count * (n + k)
        if checks > max_generator_point_checks:
            raise ValueError("generator/action-point checks exceed the rev275 preflight cap")
        ...
        payload = _payload(
            ...
            estimated_generator_point_checks=checks,
