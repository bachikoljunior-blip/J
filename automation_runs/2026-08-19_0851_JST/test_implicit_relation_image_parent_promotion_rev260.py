...
def test_rejects_returned_subgroup_outside_certified_domain_group():
    source = _image(0)
    target = _image(1)
    domain, preimage = _exact_preimage(source)
    # This transposition subgroup has the same order as the exact target image
    # subgroup but does not stabilize target U={b}. The containment gate should
    # reject any such corrupted artifact before semantic promotion.
