...
from quasipoly_recurrence_accounting_v1 import QuasipolyAccountingValidation, RecurrenceAccountingNode
from quasipoly_recurrence_accounting_v3 import _log2_factorial, _log2_sum_exp


def validate_quasipoly_recurrence_tree_v4(
    ...
    *,
    ...
    """rev210 recurrence verifier: admit exact terminal quotient fibers.

    rev150/v3 correctly required every *recursive* same-domain quotient fiber to
    expose an immediate smaller kernel-orbit partition, preventing chains of
    same-size wrappers from hiding nonprogress.  An exact terminal fiber does not
    recurse at all, so requiring an orbit child for that case rejects valid
    polynomially enumerated quotient decompositions.  v4 keeps every v3 rule and
    adds only this disjunction for `imprimitive_small_quotient` children:

      * exact certified terminal leaf; or
      * nonterminal `orbit_partition` with strictly smaller orbit children.

    This is the structural rule needed by canonical block-system-family consensus,
    where some quotient fibers can close by an existing exact terminal while the
    remaining fibers expose kernel orbits.
    """
    ...
    def rec(node, depth):
        ...
        if not node.cost_certified:
            raise Invalid("uncertified_local_cost", "local multiplicative cost lacks an execution/mechanical upper-bound certificate")
        ...
        elif node.operation_kind == "imprimitive_small_quotient":
            for edge in node.children:
                child = edge.node
                if child.n != node.n or child.m != node.m:
                    raise Invalid(
                        "invalid_imprimitive_fiber_measure",
                        "small-quotient fiber wrappers must retain the parent measure until exact termination or kernel-orbit decomposition",
                    )
                if child.terminal_certified and not child.children:
                    continue
                if child.operation_kind != "orbit_partition" or not child.children:
                    raise Invalid(
                        "unexposed_imprimitive_kernel_progress",
                        "every nonterminal quotient fiber must expose actual kernel orbit_partition children",
                    )
                if any(grand.node.m >= node.m for grand in child.children):
                    raise Invalid(
                        "nonshrinking_imprimitive_kernel_child",
                        "each exposed kernel-orbit child must be strictly smaller than the imprimitive parent domain",
                    )
        ...
        for edge in node.children:
            ...
            if node.operation_kind == "aux_shrink":
                ...
                raise Invalid(
                    ...
                    "nonterminal operation must be aux_shrink, small_aux_reset, orbit_partition, or imprimitive_small_quotient",
                )
    ...
    return QuasipolyAccountingValidation(
        ...
        nodes_checked, max_depth,
        "all proof objects are canonical, locally cost-certified, structurally progressive, and compose within the configured quasipolynomial envelope; exact same-domain quotient fibers may terminate without artificial orbit wrappers",
    )


__all__ = ["validate_quasipoly_recurrence_tree_v4"]
