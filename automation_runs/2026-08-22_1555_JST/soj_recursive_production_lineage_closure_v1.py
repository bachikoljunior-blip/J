...
def _power_of_two_cost(
    bound_value: Any, charge_value: Any, prefix: str
) -> tuple[float, float]:
    bound = _finite(
        bound_value,
        f"{prefix}.construction_multiplicative_cost_bound",
        minimum=1.0,
    )
...
def certify_recursive_production_lineage_closure(
    ...
    """Join independently replayed rev900 and rev1100 public output contracts."""
    try:
    ...
    closure_identity = _canonical_hash(payload)
    return RecursiveProductionLineageClosure(
        SCHEMA_VERSION,
        OUTPUT_STATUS,
        True,
        ...
        closure_identity,
        "rev900 total-cost lineage and rev1100 main post-replay seal replay independently and agree on one main-anchored recursive reduction/outcome/cost chain",
    )
