...
Canonical `attempt_solution` and `publish` phase-admission evidence is generated by the dedicated source-branch workflow from the claim-publication registry snapshot and persisted once without rewriting sibling evidence. Both persisted rev3400 records must report `admitted: true` and an empty conflict set before repository-wide admission can be considered satisfied.
