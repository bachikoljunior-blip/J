...
class ImplicitRelationImageResourceEnvelope:
    ...
    @property
    def phase_work_upper_bounds(self) -> tuple[tuple[str, int], ...]:
        """Return the immutable reservation split used by downstream consumers."""
        return (
            ("induced_action", self.action_work_upper_bound),
            ("domain_schreier", self.domain_schreier_work_upper_bound),
            ("image_schreier", self.image_schreier_work_upper_bound),
            ("value_coset_intersection", self.value_coset_intersection_work_upper_bound),
            ("paired_preimage", self.paired_preimage_work_upper_bound),
            ("verification", self.verification_work_upper_bound),
        )
...
def implicit_relation_image_resource_envelope(
    *,
    ...
    The envelope is independent of the concurrently owned exact execution
    modules. It can be evaluated before the exact image group exists and
    reserves induced auxiliary-generator construction, implicit domain/image
    Schreier chains, bounded value-preserving image-coset intersection, paired
    original-domain preimage reconstruction, and final exact transport and
    ...
    obligations owned by sibling leaves.
    """
    ...
    # image_order is only an upper bound. Dividing the domain-order bound by it
    # could under-reserve the kernel, so reserve the full domain order instead.
