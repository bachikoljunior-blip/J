...
The binder rejects both branches, neither branch, omitted branch slots, incomplete selected branches, mode drift, transport/original-instance drift, result/accounting drift, malformed identities, implicit boolean/integer coercions, unresolved or non-string statuses, Python string subclasses, nonliteral mapping snapshots, and unsupported root or selected-branch fields. Its output is normalized to a fixed schema and receives a deterministic SHA-256 `caller_binding_identity`.
...
The focused regression suite contains 22 cases covering valid exact-empty and exact-nonempty outcomes for both alternatives, deterministic canonical replay, tamper-sensitive binding identity, mutual exclusivity, mode mismatch, transition/original/result/accounting drift, malformed SHA identities, nonliteral boolean evidence, noninteger/negative accounting, unresolved or non-string status, unsupported shadow fields at the root and selected branch, nonliteral mapping snapshots at the root and selected branch, missing canonical branch slots, and exact-built-in-string enforcement for both values and keys.
