# Rev294 — Johnson construction cost binding

## Scope

This revision is the collision-free re-scope of the superseded rev292 cost-binding work. It closes one accounting interface between two separately owned corrected Split-or-Johnson leaves without modifying either sibling implementation.
...
- rev294 validates the retained rev287 certificate structure and its exact deterministic work formula, then replaces a potentially unit source cost with a conservative finite power-of-two upper bound at least as large as both the retained construction work and the source cost bound.
...
A naturally triggered runner for the earlier rev292 attempt exposed a separately owned pre-existing rev292 handoff-composition branch that had not appeared in the pre-claim branch search. The earlier PR was therefore closed unmerged and left untouched. A fresh rev293 claim was already present on main, so this work moved again to rev294 with a new claim, branch, run directory, workflow name, and evidence paths.

## Strict boundary

Rev294 does **not** import rev287, rev291, the sibling rev292 composition, or rev293 branch-only modules; rerun their workflows; discover a Johnson embedding; execute recursive String Isomorphism; alter a sibling certificate; merge a sibling PR; or claim corrected Split-or-Johnson/global W1R-H6/CRX/GI/AGI closure. It is only a structural/cost adapter. State remains `NOT_AGI`.
