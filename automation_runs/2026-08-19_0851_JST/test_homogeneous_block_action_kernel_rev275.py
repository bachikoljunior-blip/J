...
class BlockActionKernelRev275Test(unittest.TestCase):
    ...
    def test_preflight_generator_point_cap_fails_closed(self):
        provenance = self.provenance([[0], [1]], [(1, 0)])
        with mock.patch(
            "homogeneous_block_action_kernel_v1.schreier_stabilizer_chain",
            side_effect=AssertionError("Schreier execution must not start"),
        ):
        ...
        self.assertFalse(result.exact)
        self.assertIn("work bound", result.reason)

    def test_recorded_work_bound_covers_more_than_input_scan(self):
        provenance = self.provenance(
            [[0, 1], [2, 3]],
            [(1, 0, 2, 3), (0, 1, 3, 2), (2, 3, 0, 1)],
        )
        result = certify_block_action_kernel_factorization(provenance)
        self.assertTrue(result.exact)
        input_scan_only = 2 * result.generator_count * (
            result.domain_degree + result.block_count
        )
        self.assertGreater(result.estimated_schreier_work_units, input_scan_only)
        self.assertLessEqual(result.estimated_schreier_work_units, result.work_cap)

    def test_noninteger_resource_caps_fail_closed(self):
        provenance = self.provenance([[0], [1]], [(1, 0)])
        for cap in (True, 1.5, "100"):
            result = certify_block_action_kernel_factorization(
                provenance,
                max_generator_point_checks=cap,
            )
            self.assertFalse(result.exact)
