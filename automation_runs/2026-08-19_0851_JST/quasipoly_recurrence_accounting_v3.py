...
def validate_quasipoly_recurrence_tree_v3(
    ...
    *,
    ...
    """Proof-tree verifier with orbit and small-quotient structural composition.

    Adds two structurally checkable rules to rev150:
      * `orbit_partition`: fixed-primary, disjoint additive children whose total
        multiplicity-weighted domains fit inside the parent domain;
      * `imprimitive_small_quotient`: same-domain quotient fibers are allowed only
        when every executed fiber proof is itself an `orbit_partition` whose
        actual children are strictly smaller.  Thus same-size fiber wrappers can
        never be chained to hide nonprogress.

    The small quotient's factorial enumeration is charged in the parent node by
    the executing operator.  v3 verifies the structural progress of the exact
    proof objects; differential proof-object auditing separately checks that the
    declared local cost dominates the recorded enumeration count.
    """
    ...
    def rec(node, depth):
            ...
            for edge in node.children:
                ...
                if child.n != node.n or child.m != node.m:
                    raise Invalid(
                        ...
                        "small-quotient fiber wrappers must retain the parent measure until kernel-orbit decomposition",
                    )
                if child.operation_kind != "orbit_partition" or not child.children:
                    raise Invalid(
                        "unexposed_imprimitive_kernel_progress",
                        "every successful/empty quotient fiber must expose its actual kernel orbit_partition children",
                    )
        ...
        for edge in node.children:
            ...
            if node.operation_kind == "aux_shrink":
                ...
                # Same-size fiber wrapper is permitted only by the pre-check above;
                # recursion must immediately expose strictly smaller kernel orbits.
                pass
            else:
                raise Invalid(
    ...
    return QuasipolyAccountingValidation(
        ...
        nodes_checked, max_depth,
        "all proof objects are canonical, locally cost-certified, structurally progressive, and compose within the configured quasipolynomial envelope",
    )
